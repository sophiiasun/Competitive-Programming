...
pair<long long, int> chk(long long cc) {
    ...
    for (int x = 0; x < nn; x++) {
        ...
        while (buf.size() >= 2 && calc_top(buf[0], buf[1]) < calc_bot(buf[0], buf[1]) * 2 * points[x].second) buf.pop_front();
        if (!buf.empty()) dp[x] = min(dp[x], make_pair(dp[buf[0]].first + calc(buf[0]+1, x) + cc, dp[buf[0]].second + 1));
        while (buf.size() >= 2){
            int sz = buf.size();
            if (calc_top(buf[sz-2], buf[sz-1]) * calc_bot(buf[sz-1], x) > calc_bot(buf[sz-2], buf[sz-1]) * calc_top(buf[sz-1], x)) buf.pop_back();
            else break;
        }
...
long long take_photos(int n, int m, int k, int *r, int *c) {
    ...
    for (int x = 0; x < n; x++) {
        if (r[x] >= c[x]) swap(r[x], c[x]);
        tmp[x] = make_pair(r[x], c[x]);
    }
    sort(tmp.begin(), tmp.end(), cmp);
    for (auto &x : tmp) {
        if (!points.empty() && points.back().first <= x.first && points.back().second >= x.second) continue;
        points.push_back(x);
    }
    nn = points.size();
