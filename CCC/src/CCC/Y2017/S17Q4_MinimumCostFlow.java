...
public class S17Q4_MinimumCostFlow {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (D != 0 && adj.get(idx).s == 1 && adj.get(idx).c < D) { // MUST BE A NEW PIPE AND LESS THAN D
            ...
            par = new int[N+1];
            for (int i = 1; i <= N; i++) par[i] = i; // Initialize all node-parents as self
            for (int i = 0; adj.get(i).c < maxC || (adj.get(i).c == maxC && adj.get(i).s == 0); i++) { // Build new graph
                int n1f = find(adj.get(i).n1), n2f = find(adj.get(i).n2);
                if (n1f != n2f) par[n1f] = n2f; // Merge
            }
            for (; idx < M && adj.get(idx).c <= D; idx++) {
                if (find(adj.get(idx).n1) != find(adj.get(idx).n2)) {
